...
def main(args):
    target_prompts = []
    if args.task == "safebench_tiny":
        csv_file = os.path.join("dataset", "safebench", "question", "SafeBench-Tiny.csv")
        target_prompts = pd.read_csv(csv_file)["question"].to_list()
        print(f"len(target_prompts): {len(target_prompts)}")
    elif args.task == "advbench":
        csv_file = os.path.join("dataset", "advbench", "harmful_behaviors.csv")
        target_prompts = pd.read_csv(csv_file)["goal"].to_list()
        print(f"len(target_prompts): {len(target_prompts)}")

    kwargs = dict()
    audio_info = None
    kwargs['audio_info'] = None#audio_info
    stop_words_ids = [[151645], [151644]]
    generation_config = model.generation_config

    tgt_folder = os.path.join("exps", args.task)
    if not os.path.exists(tgt_folder):
        os.makedirs(tgt_folder)

    with open(os.path.join(tgt_folder, "infer_text.jsonl"), "w") as f_write:
        ...
        for idx, prompt in enumerate(target_prompts):
            ...
            json.dump({"idx": idx, "prompt": prompt, "response": response}, f_write)
            f_write.write("\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--task", default="safebench_tiny", type=str)
