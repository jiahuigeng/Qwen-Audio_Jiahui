...
from utils import get_logger, get_benign_data, get_target_data
import pickle
...
def main(args):
    ...
    for idx, target_prompt in enumerate(dataset):
        ...
        if (len(input_ids))> 20:
            print(target_prompt, len(input_ids))
        total_lens.append(len(input_ids))

    print(len(total_lens))
    length_frequency = Counter(total_lens)
    sorted_length_frequency = sorted(length_frequency.items())

    # 打印结果
    print("长度频率分布（从短到长）:")
    for length, frequency in sorted_length_frequency:
        print(f"长度: {length}, 出现次数: {frequency}")
    ...
    # save_file =  f"{args.task}_{args.model_name}_results_benign.csv"
    ...
    # dataset = get_benign_data(args.task)
    ...
    #     target_prompt = row["malicious"]
    #     left_prompt = row["benign"]
